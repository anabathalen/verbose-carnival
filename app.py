...
with body:
    ...
    <div class="info-card">
        ...
        <p>The relationship between protein structure and collision cross section has been the subject of lots of research. Targetted (Jarrold alpha-helix work) and untargetted (Beveridge Framework). This work has established trends in native ion mobility-mass spectrometry data, a selection of which are shown in the figure below (or will be when I have done it). By building this database, we hope to learn more about this relationship, particularly for intrinsically disordered proteins where structure prediction tools are less well developed. </p>
        
        <p>McLean and coworkers have established a similar database for small molecules measured with drift tube-mass spectrometers, and used the results to establish trends in CCS within chemical classes which can be used to filter candidate structures in untargetted workflows ([Collision Cross Section Database], https://lab.vanderbilt.edu/mclean-group/collision-cross-section-database). This database aims to establish similar trends for larger biomolecules. Databases published by Bush, Clemmer and Xu provide CCS values for standard proteins measured on drift tube-mass spectrometers used to calibrate travelling-wave IMS measurements. We hope that by broadening the criteria (including values for any protein on any IM-MS instrument), we can generate a broader overview of protein IM-MS literature, but we acknowledge the previously noted variability in published CCS values for the same 'native' protein. </p>
